...
class EnhancedMonitor13(switch.SimpleSwitch13):
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        with open("PredictFlowStatsfile.csv", "w") as fw:
            ...
            for flow in sorted([f for f in body if f.priority == 1],
                                key=lambda f: (f.match.get('eth_type'), f.match.get('ipv4_src'), f.match.get('ipv4_dst'), f.match.get('ip_proto'))):
    ...
    def _prepare_and_train(self):
        ...
        try:
            df = df.drop(columns=['timestamp', 'datapath_id', 'flow_id', 'ip_src', 'ip_dst'])
            X = df.drop(columns=['label']).astype('float64')
            y = df['label'].astype('int')
        except Exception as e:
            self.logger.error("Error preparing training data: %s", e)
            return
    ...
    def _predict_and_report(self):
        ...
        try:
            df = df.drop(columns=['timestamp', 'datapath_id', 'flow_id', 'ip_src', 'ip_dst'])
            Xp = df.astype('float64')
        except Exception as e:
            self.logger.error("Error preparing prediction data: %s", e)
            return
