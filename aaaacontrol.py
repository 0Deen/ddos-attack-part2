...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.attack_labels = {
            0: ("Legitimate", "Normal traffic", "No action needed."),
            1: ("SYN Flood", "Excessive TCP SYN packets aimed to exhaust server resources.", "Use SYN cookies, rate limiting."),
            2: ("UDP Flood", "High-volume UDP packets to random ports.", "Block unused ports, implement firewall rules."),
            3: ("ICMP Flood", "Large amounts of ICMP Echo requests (pings).", "Limit ICMP, use intrusion prevention systems."),
            4: ("HTTP GET Flood", "Many HTTP requests to overload the server.", "Use CAPTCHAs, WAFs, and rate limits."),
        }
    ...
    def flow_training(self):
        ...
        df.iloc[:, 2] = df.iloc[:, 2].astype(str).str.replace('.', '', regex=False).astype('int32')
        df.iloc[:, 3] = df.iloc[:, 3].astype(str).str.replace('.', '', regex=False).astype('int32')
        df.iloc[:, 5] = df.iloc[:, 5].astype(str).str.replace('.', '', regex=False).astype('int32')
    ...
    def flow_predict(self):
        try:
            ...
            if df.empty:
                print("No flow data found for prediction.")
                return
            ...
            for i, pred in enumerate(y_pred):
                label, desc, tip = self.attack_labels.get(pred, ("Unknown", "No description", "No tip"))
                print(f"[Flow {i}] Prediction: {label}\n  \u27a4 Description: {desc}\n  \u27a4 Mitigation: {tip}")

                with open("detected_attacks.log", "a") as log:
                    log.write(f"{datetime.now()} | Flow {i} | {label} | {desc} | {tip}\n")
