...
class SimpleMonitor13(switch.SimpleSwitch13):
    ...
    def flow_training(self):
        ...
        # Load all parquet files and the CSV file
        data_sources = [f for f in os.listdir() if f.endswith(".parquet")] + ["deen_finetune_data.csv"]
        ...
        self.flow_model = VotingClassifier(estimators=[
    ...
    def flow_predict(self):
        try:
            ...
            X_pred = df.iloc[:, :].values.astype('float64')
            y_pred = self.flow_model.predict(X_pred)

            legitimate = sum(1 for x in y_pred if x == 0)
            ...
            if (legitimate / len(y_pred) * 100) > 80:
                ...
                ip_proto = int(df.iloc[0, 7])

                ddos_type = {
                    1: "ICMP Flood",
                    6: "TCP SYN Flood",
                    17: "UDP Flood"
                }.get(ip_proto, "Unknown DDoS Attack")

                self.logger.info(f"Type of attack: {ddos_type}")
                self.logger.info("Why it occurred: Poor filtering/firewall rules or exposed IoT services")
                self.logger.info("How to prevent: Enable rate limiting, deep packet inspection, firewall hardening")
                self.logger.info("How to stop it: Isolate victim, block attacking flows, reroute traffic")
